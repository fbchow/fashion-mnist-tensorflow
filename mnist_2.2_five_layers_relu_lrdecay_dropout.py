...
mnist = input_data.read_data_sets("data/fashion", one_hot=True, reshape=False, validation_size=0)
...
B6 = tf.Variable(tf.ones([Q])/10)
W7 = tf.Variable(tf.truncated_normal([Q, 10], stddev=0.1))
...
allweights = tf.concat([tf.reshape(W1, [-1]), tf.reshape(W2, [-1]), tf.reshape(W3, [-1]), tf.reshape(W4, [-1]), tf.reshape(W5, [-1]),tf.reshape(W6, [-1]), tf.reshape(W7, [-1])], 0)
allbiases  = tf.concat([tf.reshape(B1, [-1]), tf.reshape(B2, [-1]), tf.reshape(B3, [-1]), tf.reshape(B4, [-1]), tf.reshape(B5, [-1]), tf.reshape(B6, [-1]), tf.reshape(B7, [-1])], 0)
