...
W2 = tf.Variable(tf.truncated_normal([784, M], stddev=0.1))
...
Y2 = tf.nn.relu(tf.matmul(XX, W2) + B2)
...
allweights = tf.concat([tf.reshape(W2, [-1]), tf.reshape(W3, [-1]), tf.reshape(W4, [-1]), tf.reshape(W5, [-1])], 0)
allbiases  = tf.concat([tf.reshape(B2, [-1]), tf.reshape(B3, [-1]), tf.reshape(B4, [-1]), tf.reshape(B5, [-1])], 0)
...
lr = 0.0001 +  tf.train.exponential_decay(0.0003, step, 2000, 1/math.e)
