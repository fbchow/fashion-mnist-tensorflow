...
W3 = tf.Variable(tf.truncated_normal([784, M], stddev=0.1))
...
Y3 = tf.nn.relu(tf.matmul(XX, W3) + B3)
...
allweights = tf.concat([tf.reshape(W3, [-1]), tf.reshape(W4, [-1]), tf.reshape(W5, [-1])], 0)
allbiases  = tf.concat([tf.reshape(B3, [-1]), tf.reshape(B4, [-1]), tf.reshape(B5, [-1])], 0)
